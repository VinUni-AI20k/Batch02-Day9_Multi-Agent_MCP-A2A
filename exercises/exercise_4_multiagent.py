...
def privacy_agent(state: State) -> dict:
    ...
    llm = get_llm()
    prompt = f"""Bạn là chuyên gia bảo mật dữ liệu cá nhân và GDPR. Phân tích khía cạnh bảo mật và quyền riêng tư:

Câu hỏi: {state['question']}
Phân tích pháp lý: {state.get('law_analysis', 'N/A')}

Tập trung: GDPR, rò rỉ dữ liệu (data breach), quyền riêng tư (privacy rights), hình phạt hành chính về bảo mật dữ liệu."""
...
def aggregate_results(state: State) -> dict:
    ...
    if state.get("privacy_analysis"):
        sections.append(f"🔒 PHÂN TÍCH BẢO MẬT & RIÊNG TƯ (PRIVACY):\n{state['privacy_analysis']}")
