...
LEGAL_KNOWLEDGE = [
    ...
    # TODO 1: Thêm entry về luật lao động Việt Nam
    {
        "id": "vn_labor_law",
        "keywords": ["lao động", "sa thải", "kỷ luật", "hợp đồng", "nghỉ việc"],
        "text": (
            "Theo Bộ luật Lao động 2019 của Việt Nam, người sử dụng lao động chỉ được quyền "
            "đơn phương chấm dứt hợp đồng lao động hoặc sa thải người lao động trong các trường "
            "hợp cụ thể được pháp luật quy định (như Điều 36, Điều 125). Người lao động bị sa thải "
            "trái pháp luật có quyền yêu cầu nhận lại làm việc và bồi thường thiệt hại."
        ),
    }
]
...
def check_statute_of_limitations(case_type: str) -> str:
    """Kiểm tra thời hiệu khởi kiện cho các loại vụ án khác nhau (ví dụ: 'hợp đồng', 'lao động', 'thừa kế')."""
    ...
    if "hợp đồng" in case_type_lower or "contract" in case_type_lower:
        ...
        return "Thời hiệu yêu cầu Tòa án giải quyết tranh chấp lao động cá nhân là 01 năm, kể từ ngày phát hiện ra hành vi mà mỗi bên tranh chấp cho rằng quyền, lợi ích hợp pháp của mình bị vi phạm (theo Điều 190 Bộ luật Lao động 2019)."
    elif "thừa kế" in case_type_lower or "inheritance" in case_type_lower:
        return "Thời hiệu để người thừa kế yêu cầu chia di sản là 30 năm đối với bất động sản, 10 năm đối với động sản, kể từ thời điểm mở thừa kế (theo Điều 623 Bộ luật Dân sự 2015)."
    else:
        return f"Chưa có thông tin cụ thể về thời hiệu khởi kiện cho loại vụ án: {case_type}. Vui lòng cung cấp thêm chi tiết."
...
async def main():
    ...
    # TODO 3: Thêm tool mới vào danh sách
    ...
    messages = [
        SystemMessage(content="Bạn là chuyên gia pháp lý. Sử dụng tools để tra cứu thông tin. Luôn trả lời bằng tiếng Việt."),
    ...
    if response.tool_calls:
        for tool_call in response.tool_calls:
            ...
            # TODO 4: Thêm xử lý cho check_statute_of_limitations
...
class State(TypedDict):
    ...
    privacy_analysis: Annotated[str, _last_wins]  # Đã thêm field cho Privacy Agent
    final_response: str
...
def check_routing(state: State) -> list[Send]:
    ...
    # Đã thêm logic routing cho privacy_agent
...
def privacy_agent(state: State) -> dict:
    ...
    llm = get_llm()
    prompt = f"""Bạn là chuyên gia về bảo mật dữ liệu và quyền riêng tư (Privacy Expert). Phân tích khía cạnh bảo vệ dữ liệu:

Câu hỏi: {state['question']}
Phân tích pháp lý: {state.get('law_analysis', 'N/A')}

Tập trung: Quy định GDPR, luật bảo vệ dữ liệu cá nhân (PDPA/Nghị định 13 tại VN), privacy rights, data breach (rò rỉ dữ liệu) và các hình phạt liên quan."""
...
def aggregate_results(state: State) -> dict:
    ...
    # Đã thêm privacy_analysis vào sections
...
def build_graph() -> StateGraph:
    ...
    # SỬA Ở ĐÂY: Gắn trực tiếp hàm check_routing thành luồng rẽ nhánh (conditional edge) sau law_agent
