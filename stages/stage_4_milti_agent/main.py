...
def search_privacy_law(query: str) -> str:
    ...
    knowledge = [
        (
            ...
            "Data subjects have rights: access, rectification, erasure ('right to be forgotten'), "
            "data portability, objection to processing.",
        ),
        (
            ["ccpa", "california", "consumer privacy"],
            "CCPA (California Consumer Privacy Act): Consumers have right to know, delete, "
            "and opt-out of sale of personal information. Fines: $2,500 per unintentional, "
        ...
        (
            ...
            "of a personal data breach. Article 34: Notify affected individuals without undue "
            "delay if breach poses high risk. US: 50 states have breach notification laws with "
            "varying timelines (most require 30-60 days).",
        ),
        (
            ["dpo", "data protection officer", "officer"],
            "GDPR Articles 37-39: Mandatory DPO for public authorities, organisations whose "
            "core activities require regular/systematic monitoring of individuals at large scale, "
            "or processing of special categories of data. DPO must report directly to highest management.",
        ),
    ]
    query_lower = query.lower()
    results = []
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data privacy, GDPR, CCPA, personal data, consent'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
async def call_privacy_specialist(state: LegalState) -> dict:
    """Privacy / GDPR specialist sub-agent (runs as inline ReAct agent)."""
    from langgraph.prebuilt import create_react_agent

    print("\n  [Node: call_privacy_specialist] Privacy specialist agent starting...")

    privacy_prompt = (
        "You are a data privacy and GDPR specialist with expertise in EU data protection law, "
        "CCPA, data breach notification requirements, Data Protection Officer obligations, "
